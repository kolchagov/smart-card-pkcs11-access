...
/**
 * Mapping of a native structure used by the native library. Since JNA doesn't
 * work well with structures (it has some problems with random memory
 * allocation) it is necessary to convert Java to Native and Native to Java.
 * Doing that we can achieve a single memory block with all the data correctly
 * ordered.
 * <br>
 * Also, we need to verify the conversion of Long values since it needs 8 or 4
 * bytes depending on the OS.
 * <br>
 * Got this idea from {https://github.com/joelhockey/jacknji11}
 *
 * @author jesjobom
 * http://docs.oasis-open.org/pkcs11/pkcs11-base/v2.40/os/pkcs11-base-v2.40-os.html#_Toc323024067
 *
 */
public class CK_ATTRIBUTE {
    ...
    /**
     * Inner class representing the single memory block with all the data from
     * the outter class.
     */
